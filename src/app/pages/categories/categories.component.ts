...
export class CategoriesComponent implements OnInit {
  ...
  getProductByPrice(id?: string, page?: number, min?: string, max?: string) {
    // this.callProducts(id , undefined , min, max)
  }
  // pagination(pageNumber: number) {
  //   this.callProducts(this.categoreFilterName, pageNumber, this.minValue, this.maxValue)
  // }
  // method to changet the value of the page number 
  onPageChange(event: PaginatorState) {
    this.first = event.first ?? 0;
    this.rows = event.rows ?? 10;
    let curnnetPage = Number(event?.page) + 1
    console.log(curnnetPage);
